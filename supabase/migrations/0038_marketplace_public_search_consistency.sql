...
as $$
  ...
  -- Linked models are accepted only when active and category-compatible.
  -- Only offers whose vehicle_model_id is truly null keep legacy standalone
  -- behaviour. Invalid linked offers are excluded from public results.
  base as (
    select
      v.id,
      v.vehicle_model_id,
    ...
    where v.is_active    = true
      ...
      and (
        ...
        or (
          ...
          and vm.category_id = v.category_id
        )
      )
  ...
  model_aggregated as (
    ...
    join public.vehicle_models vm
      on vm.id = mr.vehicle_model_id
     and vm.is_active = true
     and vm.category_id = mr.category_id
    where mr.rn = 1
  )
